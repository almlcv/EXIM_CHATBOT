...
def get_container_data(container_number):
    ...
    expected_columns = [
        ...
        "INVOICE VALUE AND UNIT PRICE", "BL NUMBER AND DATE", "COMMODITY", "NET WEIGHT",
        "PORT", "ARRIVAL DATE", "FREE TIME", "DETENTION FROM", "SHIPPING LINE",
        "CONTAINER NUM & SIZE", "NUMBER OF CONTAINERS", "BE NUMBER AND DATE",
        "REMARKS", "DETAILED STATUS"
    ]

    details = {col: row.get(col, "Not Available") for col in expected_columns}

    formatted_output = f"""
    📦 **Container Details for: {container_number}**
    ✅ **Job No & Date:** {details["JOB NO AND DATE"]}
    ✅ **Importer:** {details["IMPORTER"]}
    ✅ **Supplier/Exporter:** {details["SUPPLIER/ EXPORTER"]}
    ✅ **Invoice No & Date:** {details["INVOICE NUMBER AND DATE"]}
    ✅ **Invoice Value & Unit Price:** {details["INVOICE VALUE AND UNIT PRICE"]}
    ✅ **BL Number & Date:** {details["BL NUMBER AND DATE"]}
    ✅ **Commodity:** {details["COMMODITY"]}
    ✅ **Net Weight:** {details["NET WEIGHT"]}
    ✅ **Port:** {details["PORT"]}
    ✅ **Arrival Date:** {details["ARRIVAL DATE"]}
    ✅ **Free Time:** {details["FREE TIME"]}
    ✅ **Detention From:** {details["DETENTION FROM"]}
    ✅ **Shipping Line:** {details["SHIPPING LINE"]}
    ✅ **Container Size & Number:** {details["CONTAINER NUM & SIZE"]}
    ✅ **Number of Containers:** {details["NUMBER OF CONTAINERS"]}
    ✅ **BE Number & Date:** {details["BE NUMBER AND DATE"]}
    ✅ **Remarks:** {details["REMARKS"]}
    ✅ **Detailed Status:** {details["DETAILED STATUS"]}
    """
...
def get_job_data(job_number: str):
    ...
    if job_column not in df.columns:
        return f"❌ Error: Column '{job_column}' not found in the Excel file."
    ...
    if filtered_df.empty:
        return f"\n❌ No data found for job number: {job_number}\n"
    
    row = filtered_df.iloc[0]
    details = {col: row.get(col, "Not Available") for col in df.columns}
    
    formatted_output = f"""
    📂 **Job Details for: {job_number}**
    ✅ **Job No & Date:** {details.get("JOB NO AND DATE", "Not Available")}
    ✅ **Importer:** {details.get("IMPORTER", "Not Available")}
    ✅ **Supplier/Exporter:** {details.get("SUPPLIER/ EXPORTER", "Not Available")}
    ✅ **Invoice No & Date:** {details.get("INVOICE NUMBER AND DATE", "Not Available")}
    ✅ **Invoice Value & Unit Price:** {details.get("INVOICE VALUE AND UNIT PRICE", "Not Available")}
    ✅ **BL Number & Date:** {details.get("BL NUMBER AND DATE", "Not Available")}
    ✅ **Commodity:** {details.get("COMMODITY", "Not Available")}
    ✅ **Net Weight:** {details.get("NET WEIGHT", "Not Available")}
    ✅ **Port:** {details.get("PORT", "Not Available")}
    ✅ **Arrival Date:** {details.get("ARRIVAL DATE", "Not Available")}
    ✅ **Free Time:** {details.get("FREE TIME", "Not Available")}
    ✅ **Detention From:** {details.get("DETENTION FROM", "Not Available")}
    ✅ **Shipping Line:** {details.get("SHIPPING LINE", "Not Available")}
    ✅ **Container Size & Number:** {details.get("CONTAINER NUM & SIZE", "Not Available")}
    ✅ **Number of Containers:** {details.get("NUMBER OF CONTAINERS", "Not Available")}
    ✅ **BE Number & Date:** {details.get("BE NUMBER AND DATE", "Not Available")}
    ✅ **Remarks:** {details.get("REMARKS", "Not Available")}
    ✅ **Detailed Status:** {details.get("DETAILED STATUS", "Not Available")}
    """
    return formatted_output
...
async def find_container_details(container_number: str):
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Internal server error while processing container details" )
